...
function buildMainFaces() {
  ...
  addAdjacents(); // add adjacents to each smallFace
  makeUniqueEdges();
  ...
  function buildAMainFace(icoFace) {
    ...
    // center vertex
    subVertices.pCenter = pv(subVertices.p01a, subVertices.p12b, 1 / 2);
    return {
  ...
  function buildSmallFaces(mainFace) {
    const { v0, v1, v2, p01a, p01b, p12a, p12b, p20a, p20b, pCenter } =
    ...
    mainFace.smallFaces = smallTriangles.map((vertices) => ({
      vertices: vertices,
      adjacents: [],
      isVisible: false,
    }));
  ...
  function addAdjacents() {
    ...
    for (let mfi = 0; mfi < mainFaces.length; mfi++) {
      ...
      sfs[1].adjacents = [getAdjacentSF(mfi, 0, 1), sfs[2], sfs[5]];
      sfs[2].adjacents = [sfs[0], sfs[3], sfs[1]];
      sfs[3].adjacents = [sfs[2], getAdjacentSF(mfi, 1, 1), sfs[7]];
      sfs[4].adjacents = [
        getAdjacentSF(mfi, 0, 2),
        sfs[5],
        getAdjacentSF(mfi, 2, 0),
      ];
      sfs[5].adjacents = [sfs[1], sfs[6], sfs[4]];
      sfs[6].adjacents = [sfs[5], sfs[7], getAdjacentSF(mfi, 2, 1)];
      sfs[7].adjacents = [sfs[3], sfs[8], sfs[6]];
      sfs[8].adjacents = [
        sfs[7],
    ...
    function getAdjacentSF(mfi, amfi, asfi) {
      ...
      return mainFaces[target_mfi].smallFaces[EDGE_DIRS[dir][asfi]];
    }
  }
  ...
  function shareEdgeVerticesAndSphericalize() {
    ...
    for (let mfi = 0; mfi < mainFaces.length; mfi++) {
      ...
      sv.p20b = mainFaces[amfi0].subVertices[SV_ORDER[dir0][0]];
      sv.p20a = mainFaces[amfi0].subVertices[SV_ORDER[dir0][1]];

      // second adjacent MF
      const [amfi1, dir1] = MF_ADJ[mfi][1];
      sv.p01b = mainFaces[amfi1].subVertices[SV_ORDER[dir1][0]];
      sv.p01a = mainFaces[amfi1].subVertices[SV_ORDER[dir1][1]];

      // first adjacent MF
      const [amfi2, dir2] = MF_ADJ[mfi][2];
      sv.p12b = mainFaces[amfi2].subVertices[SV_ORDER[dir2][0]];
      sv.p12a = mainFaces[amfi2].subVertices[SV_ORDER[dir2][1]];
  ...
  function makeUniqueEdges() {
    for (let i = 0; i < mainFaces.length; i++) {
      const smallFaces = mainFaces[i].smallFaces;
      for (const sf of smallFaces) {
        for (let vi = 0; vi < sf.vertices.length; vi++) {
          let v0 = sf.vertices[vi];
          let v1 = sf.vertices[nti(vi + 1)];
          let newUniqueEdge;

          for (let ei = 0; ei < uniqueEdges.length; ei++) {
            if (uniqueEdges[ei].v0 === v1 && uniqueEdges[ei].v1 === v0) {
              newUniqueEdge = uniqueEdges[ei];
              break;
            }
          }
          ...
          // already exists?
          if (newUniqueEdge) {
            newUniqueEdge.smallFaces.push(sf);
          } else {
            uniqueEdges.push({ v0: v0, v1: v1, smallFaces: [sf] });
          }
        }
      }
    }
